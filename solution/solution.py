...
def streaming_chatbot() -> None:
    ...
    while True:
        ...
        if user_msg.strip().lower() in ("quit", "exit"):
            break
        messages = his + [{"role": "user", "content": user_msg}]
        ...
        for chunk in stream:
            ...
            reply += delta
        print()
        his.append({"role": "user", "content": user_msg})
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for result in results:
        prompt = str(result["prompt"])[:40]
        gpt4o_response = str(result["gpt4o_response"])[:40]
        mini_response = str(result["mini_response"])[:40]
        gpt4o_latency = f"{result['gpt4o_latency']:.3f}s"
